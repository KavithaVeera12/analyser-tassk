...
import pdf from '../component/image/images.png'
...
function FileuploadComponent() {
  ...
  const getFileIcon = (type) => {
    if (type.includes('pdf')) {
      return <img src={pdf} alt="PDF Icon" style={{ width: '35px', height: '35px',marginTop:'-17px', marginRight: '8px' }} />;
                        ...
                        <div style={{ fontSize: '2.5rem' }} className='file_imge' >{getFileIcon(file.type)}</div>
                            <div className="file-content">
                                <div className="file-details">
                                    ...
                                    <Form.Check  type="checkbox" id={`default-checkbox`} defaultChecked="true"className='checkbox' /> )  }
                                </div>
                                <div className="mr-b0 file-name mr-b5">
                                {(file.size / 1024).toFixed(2)} KB{' '}
                                </div>
                                <ProgressBar now={file.progress} label={`${file.progress}%`} className="custom-progress-bar" />
                            </div>
                        </div>
                    </Card.Body>
                  </Card>
